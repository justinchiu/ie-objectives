...
We introduce the following random variables (RVs) for each index $i \in 0, \ldots, I$,
and refer to them as word level RVs:
\begin{itemize}
\item $m_i$ indicates whether a word is a value mention.
    We have $m_i \in \set{0,1}$.
...
to give a single distribution over values for each record.
\begin{enumerate}
\setcounter{enumi}{3}
\item Aggregation $p(v \mid z,a,m) = \prod_j p(v_j \mid z,a,m)$:
    ...
    we choose the KB level value $v_j$ by aggregating our choices.
\end{enumerate}
