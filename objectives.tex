...
given a text $x = x_1, \ldots, x_{I}$ we model the facts
$\set{(e_j, t_j, v_j)}_{j=1}^J$ expressed in that text
with respect to a schema that details all entities $e_j \in \mcE$,
relation types $t_j \in \mcT$, and all values $v_j \in \mcV$.
The schema $(\mcE, \mcT, \mcV)$ gives us the possible entities, types, and values,
but not necessarily what facts appear in the KB.
We assume that the schema $(\mcE, \mcT, \mcV)$ of the KB is known at all times,
and that the schema covers all facts of interest.
The set of facts $\set{(e_j,t_j,v_j)}_{j=1}^J$ is our knowledge base (KB).
For brevity, let $e = \set{e_j}_{j=1}^J, t = \set{t_j}_{j=1}^J, v = \set{v_j}_{j=0}^J$
be the list of the entities, types, and values of the records in $r$ respectively.
The entities and types in the KB $\set{e_j, t_j}_{j=1}^J \subseteq \mcE \times \mcT$
may not contain all possible entity and type pairs.
...
we reduce the construction of $r$ to predicting, for every $j\in 1,\ldots,J$,
...
the rows of the table are given by the entities $e$,
the columns by the types $t$,
and the cells of the table take on the values $v$.
...
Alternatively, we may introduce a generative model of text $q(x \mid v)$ 
that inverts the relation extraction model $p(v \mid x)$ and optimize the following objective:
\begin{equation}
\log q(x) - KL[p(z,a,m \mid x) || q(z,a,m \mid x)]
\end{equation}
