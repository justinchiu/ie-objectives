...
$\Delta = \set{(e_j, t_j, v_j)}_{j=1}^J$ expressed in that text
with respect to a schema that details all entities $e_j$,
relation types $t_j$, and all values $v_j$.
The set of facts $\Delta$ is our knowledge base (KB).
Let $e = \set{e_j}_{j=1}^J, t = \set{t_j}_{j=1}^J, v = \set{v_j}_{j=0}^J$
