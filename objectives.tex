...
Facts therefore consist of entities, relation types, and values.
...
We focus on identifying fact mentions at the word level by identifying
value mentions, rather than entity or type mentions.
...
Once we have identified where facts are mentioned,
we must aggregate those decisions to to offer .
...
entities $p(\epsilon_i \mid x)$ and types $p(\tau_i \mid x)$.
\begin{align*}
...
and use a prior distribution over values $p(v_j \mid e_j, t_j)$.
\begin{align*}
p(v_j \mid z,a,m,e,t) &= \begin{cases}
    p(v_j \mid e_j,t_j), & \nexists i, m_i = 1 \wedge a_i = j\\
\end{cases}\\
