...
We specify the relationship between these variables with the following generative story.
\begin{enumerate}
\item Value mention $p(c \mid x) = \prod_i p(c_i \mid x)$: We choose whether word $x_i$
   is a value mention or not. Each $c_i \mid x \sim \Bern(f_c(x)_i)$, where a value of 1 indicates
...
\item Translation $p(z \mid x) = \prod_i p(z_i \mid x)$:
    ...
    The $z_i \mid x \sim \Cat(f_z(y)_i)$ is the canonical value from the KB schema 
    associated with $x_i$.
\end{enumerate}
