...
$\set{(e_j, t_j, v_j)}_{j=0}^J$ expressed in that text
...
The set of facts $\set{(e_j,t_j,v_j)}_{j=0}^J$ is our knowledge base (KB).
...
&= \sum_{z,a,m} \left(\prod_j p(v_j\mid z,a,m,x,e,t)\right) \prod_i p(z_i\mid x)p(a_i\mid x,e,t)p(m_i\mid x)\\
\end{aligned}
\end{equation}
