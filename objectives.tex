...
dimension tensor, where $g_*(\cdot)_i$ is a point on the $D$ dimensional simplex.
The conditional distributions are given by:
\begin{enumerate}
\item The value mention model $q(c \mid v,x) = g_{c}(v,x)$ 
    has access to the values $v$ from the KB, which it conditions on
    when detecting value mentions. 
\item The alignment model $q(a \mid v,x,e,t) = g_{a}(v,x,e,t)$
...
\item The translation model $q(z \mid a,v,x) = \prod_i 1(z_i = v_{a_i})$
