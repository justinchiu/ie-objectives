...
As sentences may contain many entity and value mentions, and therefore possibly
a large number of facts, we focus on identifying facts at a scale
that balances introducing ambiguity and computation complexity.
In particular, we focus on identifying the position of value mentions
...
$e$ & $\triangleq$ & The list of all entities in a KB by index. Each $e_j \in \mcE$.\\
$t$ & $\triangleq$ & The list of all types in a KB by index. Each $t_j \in \mcT$.\\
$v$ & $\triangleq$ & The list of all values in a KB by index. Each $v_j \in \mcV$.\\
\midrule
$f_*(x)$ & $\triangleq$ & A learned function whose output is of shape $|\mcI| \times D$,
    where $|\mcI|$ is the length of the word sequence and $D$ is the size of the discrete 
    distribution parameterized by this function.\\
...
$a_i$ & $\triangleq$ & A word level RV that indicates an the index of the aligned
    for word $x_i$. We have $a_i \in J$.\\
$z_i$ & $\triangleq$ & A word level RV that indicates the canonical value 
    word $x_i$ is translated to, such that $z_i \in \mcV$\\
\bottomrule
\end{tabular}
...
given a text $x = [x_0, \ldots, x_{I}]$ we must model the facts
...
\item $z_i$ indicates the canonical value word $x_i$ is a mention of,
    ...
    This is necessary as the mention of a value may not correspond exactly
    to the representation in the KB schema.
    An example is the numerical and alphabetical representation of numbers.
\end{enumerate}
...
\item Translation $p(z \mid x) = \prod_i p(z_i \mid x)$:
    We translate the word $x_i$ into a value $z_i$.
