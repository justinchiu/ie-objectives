...
is not sufficient.
Suppose we had a model that directly filled in $v \mid x,d$.
Without further assumptions, it would be difficult to separate
the contribution of the text from the marginal distribution of values.
If we are indeed interested in the extraction of facts from text,
then we must be able to answer whether facts are supported in text.
...
Since we only observe a text and associated KB, we must 
