...
Relation extraction is the task of jointly locating and converting information
conveyed in natural language into a structured representation.
By removing the noise inherent in natural language,
extraction systems produce information in a representation that is amenable to computation.
We focus on representations that can be decomposed into keys and values.

We produce an extraction system that justifies its decisions.
We use these justifications or `rationales' to bias the model away from
relying on spurious correlations that may hurt generalization.
%( garbage collection )

However, deciding what constitutes a rationale and obtaining supervision for that definition is expensive.
For every corresponding definition of a rationale, supervision must be 
obtained by having annotators consider many examples.
This prevents one from iterating quickly in the space of rationale definitions.
We therefore limit our attention to models that are able to produce rationales without supervision,
or with very limited supervision.
