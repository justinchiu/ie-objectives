...
We define the model by the following joint distribution over
...
&= \left(\prod_{j=1}^J p(v_j\mid z,a,m,x,k)\right)
\left(\prod_{i=1}^I p(z_i\mid x)p(a_i\mid x,k)p(m_i\mid x)\right)
\end{aligned}
\end{equation}
...
\item Aggregation:
    ...
    \begin{equation}
    p(v \mid z,a,m,d) = \prod_j p(v_j \mid z,a,m,k_j)
    \end{equation}
...
and $E$ an embedding function that maps keys $k$
to vectors in $\R^{d}$.
\begin{enumerate}
...
\begin{equation}
p(a_i \mid x,d) \propto \exp(E(k_{a_i})^T W_d \bh_i)\\
\end{equation}
