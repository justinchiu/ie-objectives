...
&= \underbrace{\left(\prod_{j=1}^J p(v_j\mid z,a,m,x,k)\right)}_{\textrm{Aggregation}}
    \underbrace{\left(\prod_{i=1}^I
        \underbrace{p(z_i\mid x)}_{\textrm{Translation}}
        \underbrace{p(a_i\mid x,k)}_{\textrm{Alignment}}
...
This limits the expressivity of the model, but allows us to make all choices independently
...
and uses the per-word latent variables $z,a,m$ to predict the values of the KB.
...
we align $a_i = \texttt{None}$ to a none alignment.
\begin{equation}
\begin{aligned}
...
\item Alignment: The alignment model $q(a_i \mid v,x,k)$
    uses the attention weights in Eqn.~\ref{eqn:attn} to parameterize the alignment
...
setting $m_i = 0$ for every word $x_i$.
...
$${\arg\max}_{z_i,a_i,m_i}p(z_i\mid x),p(a_i \mid x, k),p(m_i \mid x)$$
for each word $x_i$.
