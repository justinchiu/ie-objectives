...
we model the aligned knowledge base (KB) $\set{(e_j, t_j, v_j)}_{j=1}^J$
consisting of entities $e_j$,
relation types $t_j$, and all values $v_j \in \mcV$.
Let the data $D = \set{(e_j, t_j)}_{j=1}^J$ and values $v = \set{v_j}_{j=1}^J$.
The KB $(D,v)$ contains $J$ facts.
Our goal is to locate and extract facts from $x$ by modeling the values $v$
given the text $x$ and the data $D$.
...
D = \left\{\begin{array}{c}
    (e_1 = \texttt{John Doe}, t_1 = \texttt{Points}), \\
    (e_2 = \texttt{John Doe}, t_2 = \texttt{Rebounds}), \\
    (e_3 = \texttt{John Doe}, t_3 = \texttt{First name}),\\
    (e_4 = \texttt{John Doe}, t_4 = \texttt{Last name}), \\
    \vdots
\end{array}\right\},
